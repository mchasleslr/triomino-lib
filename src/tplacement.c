...
TPlacement* t_placement_cpy(TPlacement* dest, TPlacement* src) {
  if (src) {
    if (!dest) {
      dest = t_placement_create_full(
          src->player_id, t_placement_get_x(src), t_placement_get_y(src),
          t_placement_get_edge(src), src->rotation, src->triomino);
    } else {
...
char* t_placement_to_string(TPlacement* tp) {
  ...
  bytes += snprintf(str + bytes, sizeof str - bytes, "(%d, %d), ",
                    t_placement_get_x(tp), t_placement_get_y(tp));

  if (!t_placement_empty(tp)) {
    bytes += snprintf(str + bytes, sizeof str - bytes, "%s]",
                      triomino_to_string(t_placement_get_triomino(tp)));
  } else {
...
static int _get_rot_rel(TPlacement* ref, TPlacement* target) {
  return t_placement_get_rotation(ref) << 4 | t_placement_get_rotation(target)
                                                  << 6;
}
...
static int _get_pos_rel(TPlacement* ref, TPlacement* target) {
  ...
  if (x_ref == x_target && y_ref == y_target && edge_ref == !edge_target) {
    pos_rel = POS_REL_SXY;
  } else if ((x_ref == x_target + 1 || x_ref == x_target - 1) &&
             y_ref == y_target && edge_ref == !edge_target) {
    pos_rel = POS_REL_SX;
  } else if ((y_ref == y_target + 1 || y_ref == y_target - 1) &&
             x_ref == x_target && edge_ref == !edge_target) {
    pos_rel = POS_REL_SY;
  } else if (y_ref == y_target + 1 && x_ref == x_target &&
             edge_ref == edge_target) {
  ...
  } else if (x_ref == x_target + 1 && y_ref == y_target &&
             edge_ref == edge_target) {
    if (edge_ref == EDGE_INF) {
      pos_rel = POS_REL_CX1;
    } else {
  ...
  else if (x_ref == x_target + 1 && y_ref == y_target + 1 &&
           edge_ref == !edge_target) {
    if (edge_ref == EDGE_INF) {
...
bool t_placement_compatibles(TPlacement* ref, TPlacement* target) {
  ...
  t_compatibility_set_strategy(
      comp, _get_pos_rel(ref, target) | _get_rot_rel(ref, target));
