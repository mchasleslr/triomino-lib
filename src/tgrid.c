...
void t_grid_display(TGrid* grid) {
  ...
  for (i = 0; i < grid->size; i++) {
    for (j = 0; j < grid->size; j++) {
      printf("|");
      printf("%s / ",
             t_placement_to_string(t_grid_get_placement(grid, j, i, EDGE_INF)));
      printf("%s|",
             t_placement_to_string(t_grid_get_placement(grid, j, i, EDGE_SUP)));
    }
    printf("\n");
  }
}
...
TPlacement* t_grid_commit_placement(TGrid* grid, TPlacement* staged) {
  ...
  TPlacement* target = t_grid_get_placement(grid, t_placement_get_x(staged),
                                            t_placement_get_y(staged),
                                            t_placement_get_edge(staged));
...
TSet* t_grid_get_adjacents(TGrid* grid, TPlacement* ref, int options) {
  ...
  int pos_rel[] = {//          Side adjacents        //
                   POS_REL_SXY, POS_REL_SX, POS_REL_SY,
                   //         Corner adjacents       //
                   POS_REL_CCIS1, POS_REL_CCIS2, POS_REL_COPP, POS_REL_CTRANS1,
                   POS_REL_CTRANS2, POS_REL_CX1, POS_REL_CX2, POS_REL_CY1,
                   POS_REL_CY2};
  ...
  for (unsigned int i = start; i < end; i++) {
    ...
    adjacents = set_add(adjacents,
                        (TPlacement*)t_adjacency_finder_exec_strategy(finder));
  }
...
bool t_grid_valid_placement(TGrid* grid, TPlacement* candidate) {
  ...
  assert("t_grid_valid_placement: candidate rotation undefined" &&
         candidate->rotation != _UNDEFINED);
  assert("t_grid_valid_placement: candidate triomino undefined" &&
         candidate->triomino);

  bool valid = true;

  TSet* adjacents_tiles =
      t_grid_get_adjacents(grid, candidate, ADJ_DEFAULT | ADJ_FULL);
